...
Обслуживает движок виджетов из [16-widgets-and-templates.md](16-widgets-and-templates.md).
Смысл разделения: в теле документа лежит только ссылка на экземпляр виджета, а данные — в
отдельной таблице. Иначе каждый новый тип виджета менял бы схему документа и ломал бы уже
созданные записи (WID-01, WID-10).

### WIDGET_INSTANCE

| Поле | Тип | Примечание |
| --- | --- | --- |
| id | uuid | На этот идентификатор ссылается узел документа |
| owner_type / owner_id | polymorphic | `entry`, `person_comment`, `dashboard` (блок экрана «Сегодня», WID-11) |
| type | string(60) | Ключ типа из реестра: `mood`, `checklist`, `habits`, `countdown`, `gallery`, `music_disc`, `weather`, … (WID-02) |
| schema_version | int | Версия схемы данных типа (WID-10) |
| data | json | Значение виджета по схеме своего типа |
| snapshot_at | datetime | Для виджетов с внешними данными: когда снят снимок (WID-07) |
| is_collapsed | bool | Свёрнутое состояние (WID-04) |
| position | int | Порядок среди блоков-виджетов владельца |

Правила:

- Тип виджета не хранит собственных таблиц: любой новый тип добавляется записью в реестр и
  схемой поля `data`. Исключение — музыкальный диск (5.6): у него самостоятельная структура
  плейлиста, а `WIDGET_INSTANCE` лишь ссылается на диск через `data.disc_id`.
- Удаление записи-владельца переносит экземпляры в корзину вместе с ней; окончательное
  удаление удаляет и данные виджетов (5.9).
- Данные с внешним происхождением (погода, курс валюты, карта) хранятся снимком: повторное
  открытие записи не обращается к сервису и показывает погоду того дня, а не сегодняшнюю.

### WIDGET_TYPE — реестр (справочник в коде, не в БД)

Описывается в коде и версионируется вместе с приложением: ключ, название, иконка, схема
`data`, интерактивное представление, печатное представление, текстовое представление для
Markdown (WID-06), требуется ли согласие на внешние запросы (WID-09), требуется ли
разрешение устройства.

## 5.8 Диаграмма: музыка и виджеты

Выделена в отдельную схему, чтобы не перегружать диаграмму 5.1.

```mermaid
erDiagram
    USER ||--o{ MUSIC_DISC : "собирает"
    MUSIC_DISC ||--o{ MUSIC_TRACK : "содержит"
    ENTRY }o--o| MUSIC_DISC : "саундтрек"
    PERSON }o--o| MUSIC_DISC : "его музыка"

    ENTRY ||--o{ WIDGET_INSTANCE : "содержит"
    PERSON_COMMENT ||--o{ WIDGET_INSTANCE : "содержит"
    WIDGET_INSTANCE }o--o| MUSIC_DISC : "ссылается"
    WIDGET_INSTANCE }o--o{ ATTACHMENT : "использует"
    TEMPLATE_FIELD }o--o| WIDGET_INSTANCE : "порождает"
```

## 5.9 Общие правила валидации
